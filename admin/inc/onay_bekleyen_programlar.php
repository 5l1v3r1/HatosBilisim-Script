...
<div class="tab_container">
	...
	<?php
	$sayfa = g("s") ? g("s") : 1;
	$ksayisi = rows(query("SELECT prog_id FROM programlar WHERE prog_onay = 0"));
	$limit = 10;
	$ssayisi = ceil($ksayisi/$limit);
	$baslangic = ($sayfa * $limit) - $limit;
	$query = query("SELECT * FROM programlar WHERE prog_onay = 0 ORDER BY prog_id DESC LIMIT $baslangic, $limit");
