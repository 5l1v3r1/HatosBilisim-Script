...
ONAY BEKLEYEN YORUM LİSTESİ
</h3></header>
<div class="tab_container">
	
	<?php
	$sayfa = g("s") ? g("s") : 1;
	$ksayisi = rows(query("SELECT yorum_id FROM yorumlar WHERE yorum_onay = 0"));
	$limit = 10;
	$ssayisi = ceil($ksayisi/$limit);
	$baslangic = ($sayfa * $limit) - $limit;
	$query = query("SELECT * FROM yorumlar INNER JOIN konular ON konular.konu_id = yorumlar.yorum_konu_id WHERE yorum_onay = 0 ORDER BY yorum_id DESC LIMIT $baslangic, $limit");
	...
	<?php
		if ($_POST){
			...
			if (p("onay")){
				...
				echo '<h4 class="alert_success">Yorumlar başarıyla onaylandı..</h4>';
				...
				go(URL."/admin/index.php?do=onay_bekleyen_yorumlar",1);
			}
	...
	<?php }else { ?>
	<h4 class="alert_info">Onay bekleyen yorum bulunmamaktadır.</h4>
	<?php } ?>
